...
public class IevizCmd {
	...
	private RestFulCommunication rest = new RestFulCommunication();
	...
	private void run(String[] optVals) throws CommandLineException {
		...
		try {
			...
			String ofile = optVals[2];
			String inputdir = optVals[3];
			String corpus = optVals[4];
			String url = optVals[5];

			NLPTool tool = this.NLPToolMap.get(toolname);
			if (tool == null) {
				if ("moonstone".equals(toolname)) {
					System.out.println("Run:  About to create Moonstone tool");
					tool = new MoonstoneNLPTool(this, oname, ofile, inputdir, corpus, url);
	...
	public void readConfigFile() throws CommandLineException {
		try {
			...
			if (configProperties == null) {
				InputStream is = this.getClass().getResourceAsStream(File.separatorChar + ConfigFileName);
				configProperties = new Properties();
	...
	public void readMacroFile() throws CommandLineException {
		try {
			...
			if (file != null && file.exists()) {
				...
				InputStream is = this.getClass().getResourceAsStream(File.separatorChar + MacroFileName);
				String mstr = StrUtils.convertStreamToString(is);
				if (mstr != null) {
