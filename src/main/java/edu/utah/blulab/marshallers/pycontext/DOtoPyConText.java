...
//			parser.createPyConTextFiles(ontFile, modifiersFile, targetsFile, rulesFile);
	...
	/****
	 * Lee removed these 6/22/2017:  Could not compile with latest version of domain ontology.
	public void createPyConTextFiles(File ontology, File modifierFile, File targetFile, File ruleFile)
			throws Exception {
		...
		for (Modifier modifier : modDictionary) {
			...
			for (LexicalItem lexicalItem : variants) {
				...
				// once regex in modifier ontology are fixed can use match regex
				// method
				if (lexicalItem.getRegex().size() > 0) {
					...
					for (String variant : lexicalItemLabels) {
						if (!regexTuples.containsKey(variant)) {
							...
							tripleList.add(
									new Triple<String, String, String>(variant, "", lexicalItem.getActionEn(true)));
						}
					}
				}
				for (String variant : lexicalItemLabels) {
					if (!regexTuples.containsKey(variant)) {
						regexTuples.put(variant, "");
						tripleList.add(new Triple<String, String, String>(variant, "", lexicalItem.getActionEn(true)));
					}
				}

				// regexTuples.putAll(matchRegEx(lexicalItemLabels,
				// lexicalItem.getRegex()));
			}
		...
		for (Term term : anchors) {
			...
			if (regexTuples.isEmpty()) {
				// System.out.println(term.getPrefTerm() +
				// domain.getDisplayName(term.getURI()) + "" + "\t");
				for (String str : variants) {
					if (ancestor != null) {
						// System.out.println(str + "\t" +
						// domain.getDisplayName(ancestor.getURI()) + "" +
						// "\t");
				...
				for (Map.Entry<String, String> entry : regexTuples.entrySet()) {
					...
					bw.write(entry.getKey() + "\t" + domain.getDisplayName(term.getURI()) + "\t" + entry.getValue());
					bw.newLine();
				}
			}
		}
		...
		// TODO: Get variables and parse out anchor and modifiers associated
		// with each anchor
