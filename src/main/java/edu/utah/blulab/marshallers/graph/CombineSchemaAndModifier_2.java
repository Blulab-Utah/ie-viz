 ...
 */
public class CombineSchemaAndModifier_2 {

    private GraphDatabaseService graphDB = null;
    private String ontologyIRI;
    private final String schemaURI = "http://blulab.chpc.utah.edu/ontologies/v2/Schema.owl";
    private final String modifierURI = "http://blulab.chpc.utah.edu/ontologies/v2/Modifier.owl";


    public static void main(String[] args) throws Exception {
        String databaseName = "SchameModifier";
        File schemaFile = new File("C:/Users/Bill/Documents/2017/Chapman Lab/Data/ontology/Schema.owl");
        File modifierFile = new File("C:/Users/Bill/Documents/2017/Chapman Lab/Data/ontology/Modifier.owl");

//        String name = ontFile.getName();
//        name = name.substring(0, name.lastIndexOf("."));

        File graphFile = new File("C:/software_dev/Code/intellij_workspace/ie-viz/resource/" + "/db/" + databaseName);

        CombineSchemaAndModifier_2 main = new CombineSchemaAndModifier_2();
        main.createNewDB(graphFile);
        main.createDBfromOnt(schemaFile, modifierFile);
    ...
    public void createDBfromOnt(File schemaFile, File modifierFile) throws Exception {

        final OWLOntologyManager schemaManager = OWLManager.createOWLOntologyManager();
        final OWLOntology schemaOntology = schemaManager.loadOntologyFromOntologyDocument(schemaFile);
        final OWLOntologyManager modifierManager = OWLManager.createOWLOntologyManager();
        final OWLOntology modifierOntology = modifierManager.loadOntologyFromOntologyDocument(modifierFile);

        ontologyIRI = schemaOntology.getOntologyID().getOntologyIRI().toString();
        ...
        try{
            ...
            Set<OWLClass> classes = schemaOntology.getClassesInSignature(true);
            ...
            for(OWLClass cls : classes){
                ...
                Set<OWLClassExpression> superClasses = cls.getSuperClasses(schemaManager.getOntologies());
                int classCount = 0;
                ...
                Set<OWLAnnotationAssertionAxiom> annProps = cls.getAnnotationAssertionAxioms(schemaOntology);
                ...
                Set<OWLClassExpression> classExpressions = cls.getEquivalentClasses(schemaManager.getOntologies());
                classExpressions.addAll(cls.getSuperClasses(schemaManager.getOntologies()));
            ...
            //// Ontology file 2
            //get all classes in domain and imported ontologies
            Set<OWLClass> classes2 = modifierOntology.getClassesInSignature(true);

            System.out.println(ontologyIRI);
            //create node and attributes of each node
            for(OWLClass cls : classes2){
                System.out.println(cls.getIRI());
                Node newNode = getOrCreateNodeWithUniqueFactory(cls.getIRI().getShortForm(), graphDB);
                newNode.setProperty("uri", cls.getIRI().toString());

                //get superclasses to link to or else link to owl:Thing
                Set<OWLClassExpression> superClasses = cls.getSuperClasses(modifierManager.getOntologies());
                int classCount = 0;
                if(superClasses.isEmpty()){
                    newNode.createRelationshipTo(thingNode, RelationshipType.withName("IS_A"));
                }else{
                ...
                //get all annotation properties and create properties for each
                Set<OWLAnnotationAssertionAxiom> annProps = cls.getAnnotationAssertionAxioms(modifierOntology);
                for(OWLAnnotationAssertionAxiom axiom : annProps){
                    //System.out.println(axiom.toString());
                    String relType = axiom.getProperty().getIRI().getShortForm();
                    OWLLiteral value = (OWLLiteral) axiom.getValue();
                ...
                //get all axioms on each class and create relationships for each
                Set<OWLClassExpression> classExpressions = cls.getEquivalentClasses(modifierManager.getOntologies());
                classExpressions.addAll(cls.getSuperClasses(modifierManager.getOntologies()));
                for(OWLClassExpression exp : classExpressions){
                    if(exp.getClassExpressionType().equals(ClassExpressionType.OBJECT_SOME_VALUES_FROM)){
                        OWLObjectSomeValuesFrom axiom = (OWLObjectSomeValuesFrom) exp;
                        OWLObjectPropertyExpression objPropertyExpression = axiom.getProperty();
